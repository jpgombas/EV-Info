...
class DatabricksClient {
    ...
    func uploadCSVToVolume(data: [VehicleDataPoint]) async throws -> UploadResponse {
        ...
        return try await executeUpload(request: request, recordCount: data.count, path: uploadPath)
    }
    ...
    func uploadViaSQLWarehouse(data: [VehicleDataPoint]) async throws -> UploadResponse {
        ...
        return try await executeUpload(request: request, recordCount: data.count, path: tableName)
    }

    /// Upload data as JSON to volume
    ...
    /// Shared upload execution — reduces boilerplate across upload methods
    private func executeUpload(request: URLRequest, recordCount: Int, path: String) async throws -> UploadResponse {
        ...
        if httpResponse.statusCode >= 200 && httpResponse.statusCode < 300 {
            return UploadResponse(success: true,
                                ...
                                path: path,
                                message: "Successfully uploaded \(recordCount) records to \(path)")
        } else {
